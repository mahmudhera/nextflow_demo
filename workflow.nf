...
process sketch {
    ...
    if (reads.size() < 5.GB) {
        ...
        """
        fracKmcSketch ${reads[0]} ${sample_id}.sketch --ksize ${params.kmer} --scaled 1000 --fq --n ${32} --memory 10
        """
    } else {
        """
        fracKmcSketch ${reads[0]} ${sample_id}.sketch --ksize ${params.kmer} --scaled 1000 --fq --n ${64} --memory 20
        """
    }
